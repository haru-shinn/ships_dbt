...
with calculate_metrics as (
  select
    *
    ...
    , {{ safe_divide('sold_room_cnt', 'room_count') }} as room_utilization_rate

    -- 在庫消化率（人数単位）: 売れた人数 / 総定員
    , {{ safe_divide('sold_num_of_people', 'num_of_people') }} as people_utilization_rate

    -- 完売フラグ
    , case remaining_room_cnt when 0 then true else false end as is_sold_out
